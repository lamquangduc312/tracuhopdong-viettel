...
// We will locate the block from "if (currentSubscriber === 'internet') {" up to "summaryCardContainer.innerHTML = summaryHTML;"
const startToken = "if (currentSubscriber === 'internet') {";
const endToken = "            summaryCardContainer.innerHTML = summaryHTML;";

const startIndex = html.indexOf(startToken);
const endIndex = html.indexOf(endToken) + endToken.length;

if (startIndex === -1 || endIndex === -1) {
    console.error("Could not find the target block in index.html");
    process.exit(1);
}

const newBlock = `if (currentSubscriber === 'internet') {
                summaryHTML = \`
